...
function LevelBadge({ level }: { level: Course['level'] }) {
  ...
  const text: Record<Course['level'], string> = {
    Beginner: '#3A6B2E',
    Intermediate: '#8A6B00',
    Advanced: '#8A2020',
  };
  return (
    <span
      ...
      style={{ backgroundColor: colors[level], color: text[level] }}
    >
      {level}
    </span>
  );
...
function EnrollModal({ course, onClose }: { course: Course; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await enrollCourse(form.name, form.email, course.title);
      setSent(true);
    } catch {
      setError('Something went wrong. Email us at hello@skyreal.org');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        {sent ? (
          <div className="text-center py-4">
            ...
            <h3 className="text-xl font-bold mb-2" style={{ color: 'var(--primary)' }}>You&apos;re on the list!</h3>
            <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>
              We&apos;ll email you at <strong>{form.email}</strong> as soon as{' '}
              <strong>{course.title}</strong> is ready.
            </p>
          ...
          <>
            <div className="flex items-start justify-between mb-6">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: 'var(--accent)' }}>Enroll in course</p>
                <h3 className="text-xl font-bold" style={{ color: 'var(--text)' }}>{course.title}</h3>
              </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input name="email" type="email" value={form.email} onChange={handleChange} required placeholder="jane@example.com" className={inputCls} style={inputStyle} />
              </div>
              ...
              <button
                ...
                className="w-full rounded-full py-3.5 text-sm font-semibold text-white transition-all hover:opacity-90 disabled:opacity-50 mt-2"
                style={{ backgroundColor: 'var(--accent)' }}
              >
                {submitting ? 'Enrolling...' : 'Request Access'}
              </button>
              <p className="text-xs text-center" style={{ color: 'var(--text-muted)' }}>
                We&apos;ll follow up with access details within 1 business day.
              </p>
            </form>
          </>
        )}
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pb-4 flex flex-wrap gap-2">
        {course.topics.map((t) => (
          <span key={t} className="rounded-full px-3 py-0.5 text-xs" style={{ backgroundColor: 'var(--bg-card)', color: 'var(--text-muted)' }}>
            {t}
          </span>
        ))}
      </div>
      ...
      <div className="mt-auto px-6 py-4 border-t flex items-center justify-between" style={{ borderColor: '#E8EFE4' }}>
        ...
        {course.available ? (
          <button
            ...
            className="rounded-full px-4 py-2 text-xs font-semibold text-white transition-all hover:opacity-90"
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section className="relative overflow-hidden px-6 pt-32 pb-20 text-center">
        ...
        <motion.div className="relative mx-auto max-w-2xl" variants={stagger} initial="hidden" animate="show">
          ...
          <motion.h1 variants={fadeUp} className="mb-5 text-5xl font-black leading-tight">
            Learn from the<br />
            <span className="gold-text">Skyreal team.</span>
          </motion.h1>
          <motion.p variants={fadeUp} className="text-lg leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            Practical, self-paced courses on content strategy, video production, copywriting, and brand design.
            Built for busy founders and growing teams.
          </motion.p>
        </motion.div>
      </section>
      ...
      <section className="px-6 py-16">
        <div className="mx-auto max-w-6xl">
          ...
          >
            {available.map((c) => (
              <CourseCard key={c.slug} course={c} onEnroll={setSelected} />
            ))}
          </motion.div>
        </div>
      </section>
      ...
      {coming.length > 0 && (
        <section className="px-6 py-16" style={{ backgroundColor: 'var(--bg-card)' }}>
          <div className="mx-auto max-w-6xl">
            ...
            <motion.p
              className="mb-8 text-sm" style={{ color: 'var(--text-muted)' }}
              variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
            >
              These courses are in production. Enroll to join the waitlist.
            </motion.p>
            <motion.div
              className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3"
              variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
      ...
      <section className="px-6 py-20 text-center">
        <motion.div className="mx-auto max-w-xl" variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}>
          <motion.h2 variants={fadeUp} className="mb-4 text-3xl font-bold">Have a topic in mind?</motion.h2>
          <motion.p variants={fadeUp} className="mb-6 text-base" style={{ color: 'var(--text-muted)' }}>
            We&apos;re always building new curriculum. Tell us what you&apos;d love to learn.
          </motion.p>
          <motion.a
            variants={fadeUp}
            href="mailto:hello@skyreal.org?subject=Course idea"
            className="inline-block rounded-full px-8 py-4 text-base font-semibold text-white transition-all hover:opacity-90"
